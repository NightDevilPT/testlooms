...
interface CreateEnvProfileDialogProps {
  projectId: string;
  open: boolean;
...
export function CreateEnvProfileDialog({
  projectId,
  open,
  onOpenChange,
  onSuccess,
}: CreateEnvProfileDialogProps) {
  const [name, setName] = React.useState("");
  const [isDefault, setIsDefault] = React.useState(false);
  const [variables, setVariables] = React.useState<VariableRowState[]>([
    { key: "API_BASE_URL", value: "", isSecret: false, showValue: true },
  ]);
  ...
  const resetForm = () => {
    setName("");
    setIsDefault(false);
    setVariables([{ key: "API_BASE_URL", value: "", isSecret: false, showValue: true }]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await apiClient.post<EnvironmentProfileItem>(

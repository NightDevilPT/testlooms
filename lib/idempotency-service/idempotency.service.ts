...
export class IdempotencyService {
  ...
  public static async check(
    ...
    if (existingRecord) {
      ...
      if (existingRecord.completedAt && existingRecord.expiresAt > now) {
        ...
        const body = (existingRecord.responseBody ?? {}) as Record<string, unknown>;

        return {
          idempotencyKey,
          cachedResponse: NextResponse.json(body, { status: statusCode }),
        };
      }
  ...
  public static async save(
    ...
    try {
      ...
      console.error("[IdempotencyService] Failed to save record:", err);
    }
  }
  ...
  public static async release(idempotencyKey: string | null): Promise<void> {
    ...
    try {
      ...
      console.error("[IdempotencyService] Failed to release lock:", err);
    }
  }
  ...
  public static async purgeExpired(): Promise<number> {
    try {
      ...
      console.error("[IdempotencyService] Failed to purge expired records:", err);
      return 0;
    }
  }
}
